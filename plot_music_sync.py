...
def load_recordings(audio_1,audio_2):
    fs = 22050
    
    x_1.append(librosa.load(audio_1))
    x_1.append(audio_1)
    
    x_2.append(librosa.load(audio_2))
    x_2.append(audio_2)
    
    x_1.append(librosa.feature.chroma_stft(y=x_1[0], sr=fs, tuning=0, norm=2, hop_length=hop_size, n_fft=n_fft))
    x_2.append(librosa.feature.chroma_stft(y=x_2[0], sr=fs, tuning=0, norm=2, hop_length=hop_size, n_fft=n_fft))
...
def view_recordings():
    ...
    librosa.display.specshow(x_1[2], x_axis='time',
                             y_axis='chroma', cmap='gray_r', hop_length=hop_size)
    plt.colorbar()
    plt.subplot(2, 1, 2)
    plt.title('Chroma Representation of $X_2$')
    librosa.display.specshow(x_2[2], x_axis='time',
                             y_axis='chroma', cmap='gray_r', hop_length=hop_size)
...
def compare_recordings():
    fs = x_1[3]
    D, wp = librosa.sequence.dtw(X=x_1[2], Y=x_2[2], metric='cosine')
...
'''
Here we are generating the slope data.
This is one of the most important pieces of code
'''
def define_change_rates():
    slopes = []
    changerates = []
